...
local function do_fair(p, orientation)
    ...
    elseif orientation == "stable" then
        ...
        --   +---+---+---+  ->  +---+---+---+  ->  +---+---+---+
         ...
         -- How many vertical columns? Read from nmaster on the tag.
        local num_x = tonumber(termfair.stable.nmaster) or t.master_count
        local ncol  = tonumber(termfair.stable.ncol) or t.column_count
        if num_x <= 2 then num_x = 2 end
        if ncol  <= 1 then ncol  = 1 end
        local width = math.floor(wa.width/num_x)
    ...
    elseif orientation == "centermaxwidth" then
        -- Layout with fixed number of vertical columns (read from nmaster).
        -- Cols are centerded until there is nmaster columns, then windows
        -- are stacked in the slave columns, with at most ncol clients per
        -- column if possible.
        -- When there are less clients than nmaster, maxwidth will be used instead
        -- of nmaster. This allows the clients to have a larger width when the screen
        -- has not been filled yet. But then when the screen fills nmaster will be used
        -- and the clients can be smaller.

        -- with nmaster=3, ncol=1, and maxwidth=2 you'll have
        --        (1)                (2)                (3)
        --   +--+-----+--+      +-----+-----+      +---+---+---+
        --   |  |     |  |      |     |     |      |   |   |   |
        --   |  |  1  |  |  ->  |  1  |  2  | ->   | 1 | 2 | 3 |  ->
        ...
        --   +---+---+---+      +---+---+---+
         ...
         -- How many vertical columns? Read from nmaster on the tag.
        local num_x = tonumber(termfair.centermaxwidth.nmaster) or t.master_count
        local ncol  = tonumber(termfair.centermaxwidth.ncol) or t.column_count
